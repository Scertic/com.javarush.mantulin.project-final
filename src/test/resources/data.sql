...
-- sequences (на H2 необязательны; если есть — удалятся)
...
-- вместо частичного unique (WHERE ENDPOINT is null), безопасный для H2 эквивалент
...
-- ==================== REFERENCE SEED (совмещено из changelog’ов) ====================
-- PROJECT types
...
-- USER_TYPE (финальная версия)
insert into REFERENCE (CODE, TITLE, REF_TYPE) values
                                                  ('project_author','Author',5),('project_manager','Manager',5),
                                                  ('sprint_author','Author',5),('sprint_manager','Manager',5),
                                                  ('task_author','Author',5),('task_developer','Developer',5),
                                                  ('task_reviewer','Reviewer',5),('task_tester','Tester',5);

-- CONTACT types
insert into REFERENCE (CODE, TITLE, REF_TYPE) values
                                                  ('skype','Skype',0),('tg','Telegram',0),('mobile','Mobile',0),
                                                  ('phone','Phone',0),('website','Website',0),('linkedin','LinkedIn',0),('github','GitHub',0);

-- PRIORITY
insert into REFERENCE (CODE, TITLE, REF_TYPE) values
                                                  ('critical','Critical',7),('high','High',7),('normal','Normal',7),('low','Low',7),('neutral','Neutral',7);

-- MAIL_NOTIFICATION (AUX как битовая маска)
...
-- TASK_STATUS (финальная матрица переходов с ролями в AUX)
insert into REFERENCE (CODE, TITLE, REF_TYPE, AUX) values
...
-- ==================== DATA (пользователи, проекты, задачи и пр.) ====================
-- users
...
-- tasks (без описания/приоритета/оценки — их больше нет в финальной схеме)
...
-- activity
insert into ACTIVITY (AUTHOR_ID, TASK_ID, UPDATED, COMMENT, TITLE, DESCRIPTION, ESTIMATE, TYPE_CODE, STATUS_CODE, PRIORITY_CODE) values
...
-- user_belong
insert into USER_BELONG (OBJECT_ID, OBJECT_TYPE, USER_ID, USER_TYPE_CODE, STARTPOINT, ENDPOINT) values
